...
- **字体大小**：点击设置齿轮图标，调节字体 −/+
- **行距**：紧凑 / 标准 / 宽松
- **主题**：浅色（白底）/ 护眼（米黄底）/ 夜间（深色底）
- **阅读进度自动保存**：每 4 秒自动保存，关闭页面时也会保存
- **进度恢复**：重新打开同一本小说时自动滚动到上次位置
- **书架进度显示**：已读百分比显示在卡片上
- **点击正文中间**：显示/隐藏顶部栏和底部进度条
...
- 全部 PASS：核心功能通过，可以部署
- 有 FAIL：查看失败项详情，修复后重新运行

## 手机端离线缓存 v1

- 需要先在线连接电脑端同步
- 同步后书架快照自动写入 IndexedDB
- 打开过的小说正文自动缓存
- 离线时可查看缓存书架，阅读已缓存小说
- 离线时不允许上传/替换/恢复等危险操作
- 离线阅读进度暂存 IDB，下次上线后自动同步
- 未缓存正文的小说显示提示"请连接电脑端后打开"
- 电脑端仍然是唯一权威仓库
