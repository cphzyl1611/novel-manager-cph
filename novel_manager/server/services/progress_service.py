...
def _ensure_table(repo: Path) -> sqlite3.Connection:
...
def save_progress(
    ...
    conn.execute(
        """INSERT INTO reading_progress (book_id, device_id, progress_ratio, scroll_position, updated_at)
           VALUES (?, ?, ?, ?, datetime('now'))
           ON CONFLICT(book_id, device_id) DO UPDATE SET
           progress_ratio = excluded.progress_ratio,
           scroll_position = excluded.scroll_position,
           updated_at = datetime('now')""",
        (book_id, device_id, ratio, pos),
    )
    conn.commit()
    conn.close()
    return {"ok": True, "book_id": book_id, "progress_ratio": ratio, "scroll_position": pos}
